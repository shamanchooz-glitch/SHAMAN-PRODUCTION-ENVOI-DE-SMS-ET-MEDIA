const CACHE = 'shaman-production-v19';
...
self.addEventListener('fetch', (e) => {
  ...
  if (e.request.mode === 'navigate' || url.endsWith('index.html') || url.endsWith('/')) {
    ...
    // cached, from install onward. cache:'no-store' is essential here —
    // without it, the browser's own HTTP cache can silently hand back an
    // old copy even though this fetch looks like a fresh network request.
    e.respondWith(
